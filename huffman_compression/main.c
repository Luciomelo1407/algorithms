...
node *create_tree(int *histogram, int histogram_len) {
  ...
  // 1. PREENCHIMENTO DIRETO (Mantém ordem ASCII inicial)
  // Em vez de usar insert(), alocamos e jogamos direto no array
  for (int i = 0; i < histogram_len; i++) {
    if (histogram[i]) {
      node *new_node = (node *)malloc(sizeof(node));
      new_node->symble = (uint8_t)i;
      new_node->frequency = histogram[i];
      new_node->left = NULL;
      new_node->right = NULL;
  ...
  // 2. BUILD-HEAP (HEAPIFY REVERSO) - O(N)
  // Começa do último nó que tem filhos (size / 2 - 1) e vai até a raiz (0)
  for (int i = (queue->size / 2) - 1; i >= 0; i--) {
    heapify(queue->nodes, queue->size, i);
  }

  // 3. O RESTO CONTINUA IGUAL (Extração e fusão da árvore)
  while (queue->size > 1) {
    node *x = extract_min(queue);
    node *y = extract_min(queue);
    insert(queue, x->frequency + y->frequency, 0, x, y);
...
void writeLabel(FILE *out, int pos, float compressRatio) {
  fprintf(out, "%d->HUF(%0.2f%%)=", pos, compressRatio);
}
...
int main(int argc, char *argv[]) {
  ...
  int huffOutputLen = 0;
  uint8_t *huffOutput = (uint8_t *)malloc(sizeof(uint8_t) * 10000 * 2);
  ...
  for (int i = 0; i < total_input; i++) {
    ...
    huffOutputLen = huffman_compression(huffOutput, input, input_len);
    // printVector(huffOutput, huffOutputLen);

    compressRatio = (float)huffOutputLen * 100.0 / (float)input_len;

    writeLabel(file_out, i, compressRatio);
    writeVecInFile(file_out, huffOutput, huffOutputLen);
  }
